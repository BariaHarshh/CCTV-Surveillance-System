...
class TestCrowdIntegration(unittest.TestCase):
    ...
    def test_normalize_tracked_persons(self):
        ...
        p1 = normalized[0]
        self.assertEqual(p1["trackId"], 101)
        self.assertEqual(p1["confidence"], 0.92)
        self.assertEqual(p1["bbox"]["x"], round(100 / 1920, 4))
        self.assertEqual(p1["bbox"]["y"], round(200 / 1080, 4))
        self.assertEqual(p1["bbox"]["width"], round(200 / 1920, 4))
        self.assertEqual(p1["bbox"]["height"], round(400 / 1080, 4))

        # Clamped boundaries for p2
        p2 = normalized[1]
        self.assertEqual(p2["trackId"], 102)
        self.assertEqual(p2["bbox"]["x"], 0.0)
        self.assertEqual(p2["bbox"]["y"], round(100 / 1080, 4))
        self.assertEqual(p2["bbox"]["width"], 1.0)
        self.assertEqual(p2["bbox"]["height"], round((1080 - 100) / 1080, 4))

    def test_normalize_tracked_persons_empty_or_invalid(self):
        """Verify empty or zero dimension frames return empty list safely."""
        self.assertEqual(normalize_tracked_persons([], 1920, 1080), [])
        self.assertEqual(normalize_tracked_persons([{"track_id": 1, "box": (0, 0, 10, 10)}], 0, 1080), [])

    def test_crowd_detection_payload_creation(self):
        ...
        self.assertEqual(payload.organizationId, "66d550000000000000000001")
        self.assertEqual(payload.cameraId, "66d550000000000000000002")
        self.assertEqual(payload.moduleType, "OCCUPANCY_DETECTION")
        self.assertEqual(payload.metadata["rawCount"], 2)
        self.assertEqual(payload.metadata["stableCount"], 2)
        self.assertEqual(payload.metadata["currentCount"], 2)
        self.assertEqual(len(payload.metadata["people"]), 2)

    def test_publisher_throttling_and_state_change(self):
        ...
        try:
            ...
            self.assertTrue(queued_4)

        finally:
            publisher.stop()

    def test_publisher_failure_isolation(self):
        ...
        with patch("backend.services.frontend_bridge.frontend_bridge.send_detection", new_callable=AsyncMock) as mock_send:
            ...
            try:
                ...
                time.sleep(0.3)
                self.assertTrue(publisher.worker_thread.is_alive())
            finally:
                publisher.stop()

    def test_api_forward_crowd_detection_endpoint(self):
        ...
        with patch("backend.services.frontend_bridge.frontend_bridge.send_detection", new_callable=AsyncMock) as mock_send:
            ...
            self.assertEqual(data["data"]["eventId"], "evt-crowd-001")

            mock_send.assert_called_once()
            sent_payload = mock_send.call_args[0][0]
            self.assertEqual(sent_payload.moduleType, "OCCUPANCY_DETECTION")
            self.assertEqual(sent_payload.metadata["state"], "CROWD DETECTED")
